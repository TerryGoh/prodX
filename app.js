...
var updateProductListHTML = () => {
    var itemResult = "";

    console.log("updateProductListHTML") ;
...
//    localStorage["productList"] = JSON.stringify(productArray);
    ...
    for (let i = 0; i < productArray.length; i++) {
        ...
        let productCount = getProductCount(item.name);
        console.log(item);
        ...
        <div id="prod_loc" style="float:left; width:50%">Product Locations : ${productCount}</div>
        </li>`
    }
...
/*
var getProductCounts = (productName) => {
    if(!getUIMarkersArray(productName))
        return 0;
    return getUIMarkersArray(productName).length ;
}
*/

var getProductCount = (productName) => {
    if (!localStorage[productName]) 
        return 0 ;

    var prodLocArray = JSON.parse(localStorage[productName]);

    return prodLocArray.length ;
}

var updateProductCount = (productName) => {
    ...
    if (myProductName === productName) {
        ...
        var totalProducts = getProductCount(productName);
...
function handleConnectionChange(event) {
    if (event.type == "offline") {
        ...
        sessionStorage.interconnectine = "offline"
        alert("You lost connection.");
    }
    if (event.type == "online") {
        // update session
        sessionStorage.interconnectine = "online"
...
var uploadProductList2LocalStore =(productArray) => {
   // store into the localStorage
   localStorage["productList"] = JSON.stringify(productArray);
}
...
function AjaxLoadMessage() {
    ...
    $.ajax(api_url, settingsObject)
        .done(function (returnObject) {
            ...
            uploadProductList2LocalStore(returnObject.products)
            updateProductListHTML();
        })
...
var loadProductsLocations = () => {
    ...
    console.log("loadProductsLocations");
    var productName = getCurrentProductName();
...
var indexPage = () => {
    ...
    document.querySelector("#btn_updateProductList").addEventListener('click',
    () => { console.log(""); 
            AjaxLoadMessage(); 
            displayPage("page_home") }, false);
